...
interface RemittanceResponse {
  ...
  createdAt: string;
  error?: string; //  Made optional so successful responses don't break validation
}
  ...
  useEffect(() => {
    ...
    const verifyPaymentAndCreateRemittance = async () => {
      ...
      try {
        ...
        // 🛠️ Short, type-safe assertion layout to satisfy tsc build check without using 'any'
        ...
        setErrorMessage(
          err.response?.data?.message ||
            `Server authentication block or connection rejected (${err.response?.status || 'No Status'})`,
        );
      }
    };
