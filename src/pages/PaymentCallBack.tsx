...
interface RemittanceResponse {
  ...
  createdAt: string;
  error?: string; // Optional field mapping configuration
}
...
export default function PaymentCallBack() {
  ...
  // 🔒 Concurrency execution tracker flag
  ...
  useEffect(() => {
    ...
    // Terminate concurrent hooks to prevent hitting Paystack endpoints twice
    ...
    const verifyPaymentAndCreateRemittance = async () => {
      ...
      if (!token) {
        console.warn('🔐 Auth token is empty. Redirecting to login.');
      ...
      try {
        ...
        if (data.status === 'PROCESSING' || data.status === 'COMPLETED') {
          ...
          localStorage.removeItem('depositQuoteId');
          localStorage.removeItem('cachedReceiverName');
          localStorage.removeItem('cachedReceiverBankCode');
          localStorage.removeItem('cachedReceiverAccountNumber');
        } else {
          setStatus('FAILED');
          setErrorMessage(data.error || 'The transaction status returned as unserviceable.');
        }
      } catch (error) {
        console.error('Request caught in exception block:', error);
        setStatus('ERROR');

        // 🛠️ Type-safe idiomatic error resolution
        if (isAxiosError(error)) {
          setErrorMessage(
            error.response?.data?.message ||
              `Server authentication block or connection rejected (${error.response?.status || 'No Status'})`,
          );
            ...
            <div className="w-full bg-gray-50 rounded-xl p-4 border border-gray-200 text-left space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-mono text-xs text-gray-700 truncate max-w-[180px]">
                  {remittanceData.remittanceId}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Payment Reference:</span>
                <span className="font-mono font-medium text-gray-800">
                  {remittanceData.reference}
                </span>
              </div>
