...
api.interceptors.response.use(
  ...
  (error: AxiosError<unknown>) => {
    if (!error.response) {
      console.error(
        '🛑 Network/CORS Error: The server could not be reached or request was blocked by browser.',
      );
      return Promise.reject(error);
    }
    ...
    console.error(
      `⚠️ Network Interceptor caught Error Status [${status}] on Route: [${config.method?.toUpperCase()}] ${config.url}`,
    );

    if (status === 401) {
      console.error(
        '❌ 401 Unauthorized detected! Token is invalid or expired. Purging session keys.',
      );
